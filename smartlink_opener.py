#!/usr/bin/env python3
"""
Adsterra Smartlink Opener - Multi-traffic automation with proxy support
...
class AdsterraSmartlinkOpener:
    ...
    def _generate_random_profile(self, profile_index: int = 0, use_proxy: bool = True) -> Dict:
        ...
        if os_platform["name"] == "Win32" or os_platform["name"] == "Win11":
            browser = random.choice(["edge"])  # 50% Edge, 50% Chrome
        elif os_platform["name"] == "MacIntel" or os_platform["name"] == "MacAppleSilicon_M1" or os_platform["name"] == "MacAppleSilicon_M2" or os_platform["name"] == "MacAppleSilicon_M3":

            browser = random.choice(["safari"])  # 50% Safari, 50% Chrome
        else:
        ...
        return {
            "name": f"auto_profile_{profile_index}_{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
